'''
Variation of task_manager_1, with additional privileges for "admin"
...
e - Exit''')
    ...
    if menu == 'r' and active_user == "admin":    # Register a user, admin only
        ...
        if new_task[0] in users:
            ...
            with open("tasks.txt", "a") as all_tasks:
                all_tasks.write("\n" + new_task_string)
                print(f'''
The task was successfully added as follows:
{"="*70}
Task:               {new_task[1]}
Assigned to:        {new_task[0]}
Date assigned:      {new_task[3]}
Due date:           {new_task[4]}
Task complete?      {new_task[5]}
Task description:
    {new_task[2]}
{"="*70}''')
...
{"="*70}''')
    ...
    elif menu == 's' and active_user == "admin":   # Statistics, admin only
        # Could load tasks prior to user menu selection and store
        # Best approach depends on typical usage (repeated vs. single)
        ...
        with open("tasks.txt", "r") as all_tasks:
            for line in all_tasks:
                curr_task = line.split(", ")  # Standard task delimiter
                if len(curr_task) != 6:
                    n_bad_tasks += 1    # Valid tasks have six entries
                else:
                    n_valid_tasks += 1

        # Display the results; we counted users prior to login
        print(f'''
{"="*70}
Current statistics
User count:             {n_users}
Invalid user count:     {n_bad_users} 
Task count:             {n_valid_tasks}
Invalid task count:     {n_bad_tasks}
{"="*70}''')
